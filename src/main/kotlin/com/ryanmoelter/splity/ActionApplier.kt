...
class ActionApplier(
  ...
  suspend fun applyActions(actions: List<TransactionAction>) = applyActions(*actions.toTypedArray())
  ...
  private suspend fun applyCreate(action: CreateComplement) =
    with(action) {
      val transactionDescription =
        if (fromTransaction.transferAccountId != null) {
          ...
          when {
            otherSideOfTransferParent != null &&
              !otherSideOfTransferChild?.memo.isNullOrBlank() -> {
  ...
  private suspend fun applyUpdate(action: UpdateComplement): Unit =
    with(action) {
          ...
          ynab.transactions.updateTransaction(
            ...
            PutTransactionWrapper(
              SaveTransaction(
                ...
                subtransactions =
                  action.complement.subTransactions.map {
                    it.toSaveSubTransaction()
                  },
              ),
            ),
          )
  ...
  private suspend fun applyDelete(action: DeleteComplement) =
    with(action) {
      val response =
        ynab.transactions.updateTransaction(
          complement.budgetId.toString(),
  ...
  private suspend fun applyError(action: MarkError): Unit =
    with(action) {
        ...
        ynab.transactions.updateTransaction(
          ...
          PutTransactionWrapper(
            fromTransaction.toSaveTransaction().copy(
              ...
              approved = false,
              memo =
                ("ERROR: " + message + " • " + fromTransaction.memo)
...
fun PublicTransactionDetail.toSaveTransaction(): SaveTransaction =
  SaveTransaction(
...
fun PublicSubTransaction.toSaveSubTransaction(): SaveSubTransaction =
  SaveSubTransaction(
    amount = amount,
    payeeId = payeeId?.plainUuid,
    payeeName = payeeName,
  ...
  data class MarkError(
    ...
    companion object ErrorMessages {
      const val BOTH_UPDATED =
        "Both this and its complement have been updated; update both to " +
          "the same amount and date to bring them back in sync."
      const val UPDATED_TRANSFER_FROM_SPLIT =
        "Mirrors of transfers from split transactions " +
          "can't be updated, so the change has been undone. Change the mirror of this " +
          "transaction in the other budget instead."
      const val UPDATED_WITHOUT_COMPLEMENT =
        "This updated transaction has no complement; this " +
